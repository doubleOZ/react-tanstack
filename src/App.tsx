...
const AirQualityMonitor: React.FC = () => {
  ...
  const getBackgroundColor = (status?: string) => {
    return AIR_QUALITY_COLORS[status as keyof typeof AIR_QUALITY_COLORS] || AIR_QUALITY_COLORS.default;
  };
  ...
  return (
    ...
    option?.children ? String(option.children).toLowerCase().includes(input.toLowerCase()) : false
          ...
          >
            {stations?.slice().sort((a, b) => a.dustboy_name_en.localeCompare(b.dustboy_name_en)).map((station) => (
              <Select.Option
            ...
            {isAirQualityLoading ? (
              ...
              <>
                <Card title="Air Quality Details">
                  <Row gutter={16}>
                    <Col span={8}>
                      <div>PM2.5: {formatValue(airQualityData.pm25, " µg/m³")}</div>
                      <div>PM10: {formatValue(airQualityData.pm10, " µg/m³")}</div>
                    </Col>
                    <Col span={8}>
                      <div>Temperature: {formatValue(airQualityData.temp, "°C")}</div>
                      <div>Humidity: {formatValue(airQualityData.humid, "%")}</div>
                    </Col>
                  </Row>
                </Card>
                ...
                <Card 
                  ...
                  style={{ 
                    marginTop: 16, 
                    backgroundColor: getBackgroundColor(airQualityData.th_title_en),
                    color: airQualityData.th_title_en === 'Moderate' ? 'black' : 'white'
                  }}
                >
                  <div>
                    {airQualityData?.th_caption_en || "No description available"}
                  </div>
                </Card>
              </>
            ) : null}
